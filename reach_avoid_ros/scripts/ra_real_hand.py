...
control_mode = 3 # 0 for manual 1 for auto 2 for PC vs human 3 for human VS PC 
# control_mode = 1 # 0 for manual 1 for auto 
# control_mode = 2 # 0 for manual 1 for auto 
...
class Controller():
    ...
    def hvc(self):
        ...
        while not rospy.is_shutdown():
            ...
            # dxu1 = self.defender(input_d).detach().cpu().numpy()[0]
            dxu1=Fast_Catch(self.states[:3],self.states[3:])
            # dxu1=np.array([ 0.0, 0.0])wwwwwwwwwwwwwwwwwwwwwwwww
            if(self._get_done()==True):
                data=np.array([ 0.0, 0.0])
                dxu1=np.array([ 0.0, 0.0])
                print("stop!")
