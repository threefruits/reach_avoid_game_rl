...
from ra_env_def import ReachAvoidAgent_Def
...
# Agent
# action_space = spaces.Box(low=-np.array([1,1]), high=+np.array([1,1]), shape=(2,), dtype=np.float32)
agent = SAC(env.observation_space.shape[0], env.action_space, args)
# agent.load_model("models/sac_actor_ra_23.0","models/sac_critic_ra_23.0")
...
for i_episode in itertools.count(1):
    ...
    while not done:
        if args.start_steps > total_numsteps:
            ...
            # print(dp)
            # action = Fast_Catch(np.append(dp,0),state[3:])
        ...
        next_state, reward, done, _ = env.step(action,k) # Step
        episode_steps += 1
        # print(episode_steps)
        total_numsteps += 1
    ...
    if i_episode % 50 == 0:
        agent.save_model("ra", str(i_episode/50))

    if i_episode % 50 == 0 and args.eval is True:
        avg_reward = 0.
        episodes = 1
        for _  in range(episodes):
            state = env.reset(if_show_figure=False)
            episode_reward = 0
            done = False
            while not done:
                ...
                next_state, reward, done, _ = env.step(action,k)
                episode_reward += reward
